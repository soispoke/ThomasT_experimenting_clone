...
where
	trades_v2_beta.platform = new_prices.platform
	AND trades_v2_beta.tx_hash = new_prices.tx_hash
	-- These coalesces are to handle the times these values are null
	-- because in postgres NULL = NULL equals NULL :face_palm:
	AND COALESCE(trades_v2_beta.trace_address, '{-1}') = COALESCE(new_prices.trace_address, '{-1}')
	AND COALESCE(trades_v2_beta.evt_index, -1) = COALESCE(new_prices.evt_index, -1)
	AND trades_v2_beta.trade_id = new_prices.trade_id;
