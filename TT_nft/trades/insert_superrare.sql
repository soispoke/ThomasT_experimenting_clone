CREATE OR REPLACE FUNCTION nft.insert_superrare_v2_beta(start_ts timestamptz, end_ts timestamptz=now(), start_block numeric=0, end_block numeric=9e18) RETURNS integer
...
VALUES ('53 * * * *', $$
    SELECT nft.insert_superrare_v2_beta(
        (SELECT max(block_time) - interval '1 days' FROM nft.trades_v2_beta WHERE platform='SuperRare'),
        (SELECT now() - interval '20 minutes'),
        (SELECT max(number) FROM ethereum.blocks WHERE time < (SELECT max(block_time) - interval '1 days' FROM nft.trades_v2_beta WHERE platform='SuperRare')),
