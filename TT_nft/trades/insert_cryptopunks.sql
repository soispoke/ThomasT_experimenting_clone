CREATE OR REPLACE FUNCTION nft.insert_cryptopunks_v2_beta(start_ts timestamptz, end_ts timestamptz=now(), start_block numeric=0, end_block numeric=9e18) RETURNS integer
...
rows AS (
    ...
    SELECT
        ...
        trades.evt_index_array AS evt_index_array,
        tx."from" AS tx_from,
        tx."to" AS tx_to,
...
VALUES ('53 * * * *', $$
    SELECT nft.insert_cryptopunks_v2_beta(
        (SELECT max(block_time) - interval '1 days' FROM nft.trades_v2_beta WHERE platform='LarvaLabs Contract'),
        (SELECT now() - interval '20 minutes'),
        (SELECT max(number) FROM ethereum.blocks WHERE time < (SELECT max(block_time) - interval '1 days' FROM nft.trades_v2_beta WHERE platform='LarvaLabs Contract')),
